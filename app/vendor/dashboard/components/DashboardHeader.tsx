...
import { Store, RefreshCw, Bell } from 'lucide-react';
      ...
      <div className="max-w-full mx-auto px-4 sm:px-8 h-full flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-0">
        ...
        <div className="flex items-center gap-2 sm:gap-4 mt-2 sm:mt-0">
          ...
          <button
            ...
            disabled={isToggling}
            className={`relative w-12 sm:w-14 h-6 sm:h-7 rounded-full transition-all duration-300 ${
              isToggling ? 'opacity-60 cursor-not-allowed' : 'cursor-pointer'
            } ${isOpen ? 'bg-[#10B981]' : 'bg-[#EF4444]'}`}
          >
            <div
              className={`absolute top-1 w-4 h-4 sm:w-5 sm:h-5 bg-white rounded-full transition-all duration-300 shadow-md ${
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            ...
            <span className={`text-xs sm:text-sm font-medium ${isOpen ? 'text-[#10B981]' : 'text-[#EF4444]'}`}>
              {isToggling ? 'جاري التحديث...' : isOpen ? 'مفتوح' : 'مغلق'}
            </span>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 sm:gap-6 mt-2 sm:mt-0 justify-end">
          ...
          <button onClick={onRefresh} className="p-1 sm:p-2 hover:bg-[#F3F4F6] rounded-lg transition-colors">
            <RefreshCw className="w-4 sm:w-5 h-4 sm:h-5 text-[#6B7280]" />
          </button>

          <button className="relative p-1 sm:p-2 hover:bg-[#F3F4F6] rounded-lg transition-colors">
            <Bell className="w-4 sm:w-5 h-4 sm:h-5 text-[#6B7280]" />
            {notifications > 0 && (
              <span className="absolute -top-1 -right-1 w-4 sm:w-5 h-4 sm:h5 bg-[#EF4444] text-white text-[8px] sm:text-xs font-bold rounded-full flex items-center justify-center animate-pulse">
                {notifications}
              </span>
            )}
          </button>

...
import { Clock, MapPin, Phone, MoreVertical, Eye, X, ChevronDown } from 'lucide-react';
...
export function OrderCard({ order, onStatusChange, onViewDetails, onCancel }: OrderCardProps) {
  ...
  return (
    ...
    >
      {/* Header Row - مُحسّن للشاشات الصغيرة */}
      ...
      {/* Main Content - Stack على الموبايل الصغير */}
      <div className="flex flex-col gap-3 sm:gap-4 md:gap-6 p-3 sm:p-4 md:p-5">
        {/* Customer Info - مُبسط على الشاشات الصغيرة */}
      ...
      {/* Actions Row - مُحسّن بالكامل للشاشات الصغيرة */}
      <div className="bg-gradient-to-br from-[#F9FAFB] to-[#F3F4F6] px-2.5 sm:px-3 md:px-5 py-2.5 sm:py-3 md:py-4 border-t-2 border-[#E5E7EB]">
        ...
        <div className="hidden md:flex items-center justify-between gap-4">
          ...
          <div className="relative">
            <button
              ...
              className="px-6 py-3 bg-gradient-to-r from-[#E5A04D] to-[#D4903D] text-white rounded-xl font-semibold text-sm hover:shadow-lg hover:scale-105 transition-all duration-200 min-w-[180px] flex items-center justify-center gap-2 shadow-md"
            >
            ...
            {showStatusDropdown && (
              <>
                ...
                />
                <div className="absolute right-0 bottom-full mb-2 w-52 bg-white rounded-xl shadow-lg border border-[#E5E7EB] z-50 py-2">
                  {(Object.entries(statusConfig) as [OrderStatus, typeof statusConfig[OrderStatus]][]).map(([key, config]) => (
                    ...
                    >
                      <span className="flex items-center gap-2 min-w-0">
                        ...
                        <span className="truncate" style={{ color: order.status === key ? config.color : '#1A1A1A' }}>
                          {config.label}
                        </span>
                      </span>
                      {order.status === key && <span className="text-[#10B981] flex-shrink-0">✓</span>}
                    </button>
                  ))}
                </div>
              </>
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={() => onViewDetails(order)}
              className="px-4 py-2.5 bg-white border-2 border-[#E5E7EB] text-[#1A1A1A] hover:border-[#E5A04D] hover:bg-[#FEF3E2] hover:text-[#E5A04D] rounded-xl text-sm font-semibold flex items-center gap-2 transition-all duration-200 shadow-sm hover:shadow whitespace-nowrap"
            >
            ...
            {(order.status === 'pending' ) && (
              <button
                ...
                className="px-4 py-2.5 bg-white border-2 border-[#FEE2E2] text-[#EF4444] hover:bg-[#FEE2E2] hover:border-[#EF4444] rounded-xl text-sm font-semibold flex items-center gap-2 transition-all duration-200 shadow-sm hover:shadow whitespace-nowrap"
              >
        ...
        {/* Mobile Layout (below md) - مُحسّن للشاشات الصغيرة جداً */}
        <div className="md:hidden space-y-2">
          {/* Status Update Button - Full Width */}
          <div className="relative w-full">
            <button
              onClick={() => setShowStatusDropdown(!showStatusDropdown)}
              className="w-full px-3 sm:px-4 py-2.5 sm:py-3 bg-gradient-to-r from-[#E5A04D] to-[#D4903D] text-white rounded-lg sm:rounded-xl font-semibold text-xs sm:text-sm hover:shadow-lg transition-all duration-200 flex items-center justify-center gap-1.5 sm:gap-2 shadow-md"
            >
              <span>تحديث الحالة</span>
              <ChevronDown className={`w-3.5 sm:w-4 h-3.5 sm:h-4 transition-transform flex-shrink-0 ${showStatusDropdown ? 'rotate-180' : ''}`} />
            </button>
            ...
            {showStatusDropdown && (
              <>
                <div
                  ...
                  onClick={() => setShowStatusDropdown(false)}
                />
                <div className="absolute right-0 bottom-full mb-2 w-full bg-white rounded-lg sm:rounded-xl shadow-lg border border-[#E5E7EB] z-50 py-1.5 sm:py-2">
                  {(Object.entries(statusConfig) as [OrderStatus, typeof statusConfig[OrderStatus]][]).map(([key, config]) => (
                    <button
                      key={key}
                      onClick={() => {
                      ...
                      className="w-full px-3 sm:px-4 py-2.5 sm:py-3 text-right text-xs sm:text-sm hover:bg-[#F8FAFC] flex items-center justify-between gap-2 transition-colors"
                      style={{
                        backgroundColor: order.status === key ? config.bg : 'transparent',
                      }}
                    >
                      <span className="flex items-center gap-1.5 sm:gap-2 min-w-0">
                        <span className="flex-shrink-0 text-sm sm:text-base">{config.icon}</span>
                        <span className="truncate text-xs sm:text-sm" style={{ color: order.status === key ? config.color : '#1A1A1A' }}>
                          {config.label}
                        </span>
                      </span>
                      {order.status === key && <span className="text-[#10B981] flex-shrink-0 text-sm">✓</span>}
                    </button>
                  ))}
                </div>
              </>
          ...
          {/* Secondary Actions - Grid على الموبايل */}
          <div className="grid grid-cols-2 gap-1.5 sm:gap-2">
            <button
              onClick={() => onViewDetails(order)}
              className="px-2 sm:px-3 py-2 sm:py-2.5 bg-white border-2 border-[#E5E7EB] text-[#1A1A1A] hover:border-[#E5A04D] hover:bg-[#FEF3E2] hover:text-[#E5A04D] rounded-lg sm:rounded-xl text-xs sm:text-sm font-semibold flex items-center justify-center gap-1 sm:gap-1.5 transition-all duration-200 shadow-sm"
            >
              <Eye className="w-3.5 sm:w-4 h-3.5 sm:h-4 flex-shrink-0" />
              <span>التفاصيل</span>
            </button>
            {(order.status === 'pending') && (
              <button
                onClick={() => onCancel(order.id)}
                className="px-2 sm:px-3 py-2 sm:py-2.5 bg-white border-2 border-[#FEE2E2] text-[#EF4444] hover:bg-[#FEE2E2] hover:border-[#EF4444] rounded-lg sm:rounded-xl text-xs sm:text-sm font-semibold flex items-center justify-center gap-1 sm:gap-1.5 transition-all duration-200 shadow-sm"
              >
                <X className="w-3.5 sm:w-4 h-3.5 sm:h-4 flex-shrink-0" />
                <span>إلغاء</span>
              </button>

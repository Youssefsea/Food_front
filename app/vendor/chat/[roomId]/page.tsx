...
export default function VendorChatRoomPage() {
  ...
  useEffect(() => {
    ...
    // تأكد من استبدال المفتاح إذا كان مختلفاً في ملفات البيئة الخاصة بك
    const pusher = new Pusher('39ade55f3979c3c6e71b', { cluster: 'eu' });
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    try {
      // إرسال الرسالة إلى الـ API الذي سيقوم بدوره ببثها عبر Pusher
      await api.post(`/room/${roomId}/message`, { message });
    } catch {
      toast.error('فشل إرسال الرسالة، حاول مرة أخرى');
      // إعادة الرسالة إلى حقل الإدخال في حالة الفشل

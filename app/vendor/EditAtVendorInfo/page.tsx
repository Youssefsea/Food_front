...
export default function EditRestaurantSettings() {
  ...
  const fetchProfile = useCallback(async () => {
    try {
      ...
      const data: RestaurantData = {
        ...
        allowed_radius_km: profile.allowed_radius_km || 15,
  ...
  const handleSave = async () => {
    try {
      ...
      const deliveryFeeChanged = formData.delivery_fees !== originalData.delivery_fees;


      const promises: Promise<unknown>[] = [];

      if (infoChanged || deliveryFeeChanged) {
        promises.push(
      ...
      const error = err as { response?: { data?: { message?: string } } };
      setError(error.response?.data?.message || 'حدث خطأ في حفظ التغييرات');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC]" dir="rtl">
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-6">
            ...
            {tabs.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`flex-1 min-w-35 flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-sm font-medium transition-all ${
                    activeTab === tab.id
        ...
        {hasChanges && (
          <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-[#E5E7EB] p-4 sm:hidden z-50 animate-slide-up">
            <div className="flex gap-3">
              <button
                onClick={handleReset}
                className="flex-1 h-12 border-2 border-[#E5E7EB] text-[#6B7280] rounded-xl font-semibold hover:bg-[#F3F4F6] transition-colors"
              >
                إلغاء
              </button>
              <button
                onClick={handleSave}
                disabled={saving}
                className="flex-1 h-12 bg-[#E5A04D] text-white rounded-xl font-semibold hover:bg-[#D4922F] transition-colors flex items-center justify-center gap-2 disabled:opacity-50"
              >
                {saving ? (
